...
def join_journal_article_metadata_with_sjr():
    ...
    log_dataframe(sjr_impact_df)            # Shape: 46877x5
    ...
    log_dataframe(joined_df)               # Shape: 5573x45

    # Failed joins: Issn = None
    print(joined_df.isna().sum()) # Issn = 313. 313 failed joins. [successful joins = 5573-313 = 5260]

    # Issn is Valid but SJR is None
    joins_with_no_sjr = joined_df[
        ~joined_df['Issn'].isnull() & joined_df["SJR"].isna()
    ]
    print("SHAPE:")
    print(joins_with_no_sjr.shape) # 4x5 (4 such records where Issn is Valid but SJR is None)
    print(joins_with_no_sjr.to_string())
...
def join_outputs_metadata_with_sjr():
    ...
    log_dataframe(joined_df)               # Shape: 7296x45
...
join_journal_article_metadata_with_sjr()
join_outputs_metadata_with_sjr()
